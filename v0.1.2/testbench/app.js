...
(function (tsumego) {
    ...
    (function (mgen) {
        ...
        function fixed(board, target) {
            var maxsize = arguments.length <= 2 || arguments[2] === undefined ? 25 : arguments[2];

            var sa = new mgen.MvsOrd(board);
            ...
            //console.log('#5', rzone + '');
            if (rzone.stones.length > maxsize) throw new Error("The number of possible moves " + rzone.stones.length + " is more than " + maxsize);
            return function (color) {
                var moves = sa.reset();
                for (var move of rzone.stones) {
...
(function (tsumego) {
    ...
    var TT = (function () {
        ...
        TT.prototype.get = function get(hash, color, nkt) {
            var s = this.data[hash];
            if (!s) return 0;
            var e = entry.get(s, color);
            var winner = nkt >= entry.b(e) ? +1 : nkt <= entry.w(s) ? -1 : 0; // not solved for this number of ko treats
            if (!winner) return 0;
        ...
        TT.prototype.set = function set(hash, color, move, nkt) {
            if (nkt < -2 || nkt > +2 || !tsumego.stone.color(move)) throw Error('Invalid TT entry.');
            var s = this.data[hash] || ++this.size && entry.base;
            var e = entry.get(s, color);
            var hc = tsumego.stone.hascoords(move);
            var x = tsumego.stone.x(move);
            var y = tsumego.stone.y(move);
            var b = entry.b(e);
            var w = entry.w(e);
...
(function (tsumego) {
    ...
    (function (solve_1) {
        ...
        function* start(args) {
            var _ref10 = typeof args === 'string' ? parse(args) : args;

            var board = _ref10.root;
            var color = _ref10.color;
            var _ref10$nkt = _ref10.nkt;
            var nkt = _ref10$nkt === undefined ? 0 : _ref10$nkt;
            var _ref10$tt = _ref10.tt;
            var tt = _ref10$tt === undefined ? new tsumego.TT() : _ref10$tt;
            var log = _ref10.log;
            var expand = _ref10.expand;
            var status = _ref10.status;
            var alive = _ref10.alive;
            var stats = _ref10.stats;
            var unodes = _ref10.unodes;
            ...
            var tags = []; // tags[i] = hash of the path to the i-th position
            function* solve(color, nkt) {
                ...
                var ttres = tt.get(hashb, color, nkt);
                ...
                var guess = tt.move[hashb ^ color] || 0;
                ...
                for (var _move2 of nodes) {
                    ...
                    var h = tsumego.gf32.mul(prevb != hashb ? prevb : 0, 0x87654321) ^ hashb;
                    tags.push(h & ~15 | (nkt & 7) << 1 | (color < 0 ? 1 : 0));
                    path.push(hashb);
                    stats && stats.nodes++;
                    if (!_move2) {
                        debug && (yield 'yielding the turn to the opponent');
                        var i = tags.lastIndexOf(tags[depth], -2);
                        if (i >= 0) {
    ...
    (function (GobanElement) {
        function create(board) {
            ...
            Object.assign(svg, {
                ...
                getStoneCoords: function getStoneCoords(event) {
...
(function (testbench) {
    ...
    testbench.vm = Object.defineProperties({}, {
        tool: {
            ...
            get: function get() {
                var rb = document.querySelector('input[name="tool"]:checked');
                return rb && rb.value;
            },
            set: function set(value) {
                var rbs = document.querySelectorAll('input[name="tool"]');
                for (var rb of rbs) {
                    if (rb.getAttribute('value') == value) rb.setAttribute('checked', '');else rb.removeAttribute('checked');
                }
            },
            configurable: true,
            enumerable: true
        ...
        solveFor: {
            ...
            get: function get() {
                var rb = document.querySelector('input[name="solve-for"]:checked');
        ...
        nkt: {
            ...
            get: function get() {
                var input = document.querySelector('input[name="nkt"]');
...
(function (testbench) {
    ...
    function solve(op, board, color) {
        var nkotreats = arguments.length <= 3 || arguments[3] === undefined ? 0 : arguments[3];
    ...
    window.addEventListener('load', function () {
        Promise.resolve().then(function () {
            ...
            window.addEventListener('hashchange', function () {
                ...
                if (path.length > 0) {
                    loadProblem(path)["catch"](function (e) {
                        console.log(e.stack);
            ...
            testbench.ls.added.push(function (path) {
                for (var e of directory.querySelectorAll('.entry')) {
                    var a = e;
                    if (a.hash == '#' + path) return;
                }
                addDirEntry(path);
            });
                ...
                for (var dir of manifest.dirs) {
                    var _loop = function (path) {
                        send('GET', '/problems/' + path).then(function (sgf) {
                            ...
                            for (var nvar = 0; nvar <= root.vars.length; nvar++) {
                                var _name6 = path.replace('.sgf', '') + (nvar ? ':' + nvar : '');
                                if (!lsdata[_name6]) addDirEntry(_name6);
                            }
                        })["catch"](function (err) {
            ...
            document.querySelector('#solve').addEventListener('click', function (e) {
                if (!testbench.vm.solveFor) {
                    alert('Choose which side plays first');
                    return;
                }
                lspath = null;
                if (testbench.vm.debugSolver) dbgsolve(board, testbench.vm.solveFor, testbench.vm.nkt);else solveAndRender(testbench.vm.solveFor, testbench.vm.nkt);
            });
            ...
            document.querySelector('#download').addEventListener('click', function (e) {
                ...
                var blob = new Blob([getProblemSGF()], { type: 'application/x-go-sgf' });
                a.href = URL.createObjectURL(blob);
                a['download'] = /[^\/]+$/.exec(lspath)[0] + '.sgf';
                a.click();
            });
            document.querySelector('#getsvg').addEventListener('click', function (e) {
                var div = document.querySelector('.tsumego');
                var a = document.createElement('a');
            ...
            var sgfinput = document.querySelector('#sgf');
    ...
    function loadProblem(path) {
        return Promise.resolve().then(function () {
            ...
            return Promise.resolve().then(function () {
                ...
                for (var e of document.querySelectorAll('.directory .entry')) {
                    var a = e;
                    if (a.hash == '#' + path) a.classList.add('selected');else a.classList.remove('selected');
                }
            });
        });
    }
    ...
    function renderBoard() {
        ...
        if (stone.hascoords(move)) ui.TR.add(stone.x(move), stone.y(move));
        ...
        ui.addEventListener('mousedown', function (event) {
            if (event.ctrlKey) {
                ...
                var _ui$getStoneCoords = ui.getStoneCoords(event);

                sel0x = _ui$getStoneCoords[0];
                sel0y = _ui$getStoneCoords[1];
            }
        });
        ui.addEventListener('mouseup', function (event) {
            if (event.ctrlKey && sel0x >= 0 && sel0y >= 0) {
                var _ui$getStoneCoords2 = ui.getStoneCoords(event);

                var sel1x = _ui$getStoneCoords2[0];
                var sel1y = _ui$getStoneCoords2[1];

                for (var x = Math.min(sel0x, sel1x); x <= Math.max(sel0x, sel1x); x++) {
                    for (var y = Math.min(sel0y, sel1y); y <= Math.max(sel0y, sel1y); y++) {
        ...
        ui.addEventListener('mousemove', function (event) {
            var _ui$getStoneCoords3 = ui.getStoneCoords(event);

            var x = _ui$getStoneCoords3[0];
            var y = _ui$getStoneCoords3[1];

            var s = stone(x, y, 0);
            document.title = document.title.replace(/^(.+?)( - [a-z][a-z] - [A-Z]\d+)?$/, "$1 - " + stone.toString(s) + " - " + stone.cc.toString(s, board.size));
        });
        ui.addEventListener('click', function (event) {
            event.preventDefault();
            event.stopPropagation();

            var _ui$getStoneCoords4 = ui.getStoneCoords(event);

            var x = _ui$getStoneCoords4[0];
            var y = _ui$getStoneCoords4[1];

            var c = board.get(x, y);
            if (!lspath) {
                var s = stone(x, y, -testbench.vm.solveFor);
                var r = board.play(s);
                if (!r) console.log(stone.toString(s) + ' cannot be played on this board:\n' + board);
            } else switch (testbench.vm.tool) {
                case 'MA':
                    // mark the target                   
                    aim = stone(x, y, 0);
                    break;
        ...
        var editor = document.querySelector('.tsumego-sgf');
    ...
    function solveAndRender(color) {
        var nkt = arguments.length <= 1 || arguments[1] === undefined ? 0 : arguments[1];
        ...
        setTimeout(function () {
            ...
            var op = {
                notify: function notify() {
                    ...
                    var comment = 'Solving... elapsed ' + (duration / 1000).toFixed(1) + 's; cached ' + testbench.tt.size + ' positions';
                    setComment(comment);
                }
            };
            solve(op, board, color, nkt, true).then(function (move) {
                var duration = Date.now() - started;
                if (!stone.hascoords(move) || move * color < 0) {
                    ...
                    board.play(move);
                    console.log(board + '');
                    var comment = stone.toString(move) + ' in ' + (duration / 1000).toFixed(1) + 's; cached ' + testbench.tt.size + ' positions';
                    renderBoard(comment);
                }
            })["catch"](function (err) {
                setComment(err);
