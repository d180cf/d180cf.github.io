...
(function (tsumego) {
    ...
    (function (mgen) {
        ...
        // dS / dx = (S / x)**2
        const S = (x) => x / (1 + tsumego.sign(x) * x);
        ...
        class MvsOrd {
            ...
            insert(x, y, color) {
                ...
                if (0 && tsumego.isDumb(board, x, y, color, this.safe))
                    return false;
                const s = tsumego.stone.make(x, y, color);
                const r = board.play(s);
                if (!r)
                    return false; // the move is not playable
                ...
                this.sa.insert(s, [
                    ...
                    +w[0] * S(r)
                        + w[1] * S(mgen.ninatari(board, +color))
                        + w[2] * S(n * color * tsumego.sign(t))
                        + w[3] * S(mgen.sumlibs(board, +color))
                        + w[4] * S(mgen.ninatari(board, -color))
                        + w[5] * S(mgen.sumlibs(board, -color))
                        + w[6] * S(tsumego.random() - 0.5)
                ]);
                board.undo();
                return true;
            }
...
(function (tsumego) {
    ...
    (function (mgen) {
        ...
        function fixed(board, target) {
            ...
            const sa = new mgen.MvsOrd(board, target, s => {
...
(function (tsumego) {
    ...
    (function (solve_1) {
        ...
        function* start(args) {
            let { board, color, km, tt = new tsumego.TT, log, expand, target, alive, debug, time } = typeof args === 'string' ? parse(args) : args;
            if (log && alive) {
                const test = alive;
                alive = node => {
                    const res = test(node);
                ...
                for (const move of expand(color)) {
                    ...
                    sa.insert(tsumego.repd.set(move, d), [
                        ...
                        +1e-0 * d
                            ...
                            + 1e-2 * tsumego.sign(tt.move.get(hash ^ -color) * color)
                    ]);
                }
