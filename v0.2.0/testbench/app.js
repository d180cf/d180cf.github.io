        ...
        function* start(args) {
            ...
            function* solve(color, km) {
                ...
                for (var _move2 of nodes) {
                    ...
                    stats && stats.nodes++;
                    if (!_move2) {
                        debug && (yield tsumego.stone.nocoords(color) + ' plays elsewhere');
...
(function (testbench) {
    ...
    var rs = +(/\brs=([+-]?\d+)\b/.exec(location.search) || [])[1] || Date.now() | 0;
    ...
    function renderBoard() {
        ...
        ui.addEventListener('mousemove', function (event) {
            ...
            var s = stone(x, y, 0);
            testbench.vm.coords = stone.toString(s) + " - " + stone.cc.toString(s, board.size);
        });
