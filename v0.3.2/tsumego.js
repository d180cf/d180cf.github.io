    ...
    (function (mgen) {
        class MvsOrd {
            constructor(board, target) {
                ...
                this.sa = new tsumego.SortedArray((a, b) => {
...
(function (tsumego) {
    ...
    (function (solve_1) {
        ...
        function* start(args) {
            let { board, color, km, tt = new tsumego.TT, log, expand, target, alive, stats, unodes, debug, time } = typeof args === 'string' ? parse(args) : args;
            if (log && alive) {
                const test = alive;
                alive = node => {
                    const res = test(node);
            ...
            const sa = new tsumego.SortedArray((a, b) => {
                for (let i = 0; i < a.length; i++) {
                    const d = b[i] - a[i];
                    if (d)
                        return d;
